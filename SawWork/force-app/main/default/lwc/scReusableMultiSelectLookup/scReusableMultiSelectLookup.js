...
export default class ScReusableMultiSelectLookup extends LightningElement {
    // Public properties
    @api label;
    @api required;
    @api placeholder = '';
    ...
    /*@api 
    set taskrecordid(curTaskRecordId){
        this.taskRecId = curTaskRecordId;
    }
    ...
    */
    @api
    set objrecordid(curObjRecordId){
        this.objRecId=curObjRecordId;
    }

    get objrecordid(){
        return this.objRecId;
    }
    ...
    handleResultClick(event) {
        ...
        this.dispatchSelectionSelect();
    }

    dispatchSelectionSelect() {
        this.dispatchEvent(new CustomEvent('selectionadd', { detail: this._curSelection.map((sel) => sel.id) }));
    }

    dispatchSelectionRemove() {
        this.dispatchEvent(new CustomEvent('selectionremove', { detail: this._curSelection.map((sel) => sel.id) }));
    }
    ...
    handleSubmit() {
        ...
        this.dispatchEvent(new CustomEvent('submit', { detail: this._curSelection.map((sel) => sel.id) }));
        //this.loadSpinner = true;
    ...
    handleModal(event){
        ...
        event.preventDefault();
        this.recordToDelete = event.currentTarget.name;
