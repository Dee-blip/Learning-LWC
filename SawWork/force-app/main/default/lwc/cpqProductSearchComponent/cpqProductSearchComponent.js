...
export default class cpqProductSearchComponent extends NavigationMixin(LightningElement) {
// Public properties
        ...
        if (this._cleanSearchTerm.length >= this.minSearchTermLength) {
            ...
            const searchEvent = new CustomEvent('search', {
                detail: {
                    ...
                    selectedmlcids : this._curSelection.map((sel) => sel.subtitle),
...
processSelectionUpdate(isUserInteraction) {
    ...
    if (isUserInteraction) {
        ...
        const selectedmlcids = this._curSelection.map((sel) => sel.subtitle);
        
        this.dispatchEvent(new CustomEvent('selectionchange', { detail: selectedmlcids }));
...
handleResultClick(event) {
    ...
    let curvalue = JSON.stringify(selectedItem.subtitle);
    if (!selectedItem) {
        return;
    }
    const newSelection = [...this._curSelection];
    const index = newSelection.findIndex((e) => e.subtitle === selectedItem.subtitle);
    if (index === -1) 
            {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        ...
                        message:'Marketing Id already exits :- ' +curvalue,
                        variant: 'error'
                    }),
                );
    ...
    console.log('selected product on selection from existing records'+this._curSelection);
...
handleRemoveSelectedItem(event) {
    ...
    const subtitle = event.currentTarget.name;
    this._curSelection = this._curSelection.filter((item) => item.subtitle !== subtitle )
    // Process selection update
    this.processSelectionUpdate(true);
    // Handle Invalid M-lc-ids
...
handleNewRecordClick(event) {
        ...
        let selectName = event.currentTarget.dataset.id.toUpperCase();
        let newsObject ={'subtitle':selectName,'icon':"utility:product_service_campaign" };
        let curvalue = JSON.stringify(newsObject.subtitle);
        
            const index = this._curSelection.findIndex((e) => e.subtitle === newsObject.subtitle);
            if (index === -1) 
            {
                this._curSelection.push(newsObject);
            } 
            else {
                this.dispatchEvent(
                    new ShowToastEvent({
            ...
            console.log('selected product on selection from new records'+ JSON.stringify(this._curSelection));
