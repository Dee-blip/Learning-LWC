...
export default class Hd_MultiSelectPickList extends LightningElement {
    ...
    set options(value) {
        ...
        let optionBeforeSort = [];
        optionBeforeSort = optionBeforeSort.concat(value);

        // this.inputOptions = options.concat(value);
        optionBeforeSort.sort(function compare(a, b) {
            if (a.value === 'All') {
                return -1;
        ...
        this.inputOptions = options.concat(optionBeforeSort);
    }
    @api
    clear(){
    ...
    handleOption(event, value){
        ...
        if (listBoxOption.classList.contains("slds-is-selected")) {
            ...
            let option = this.options.find(option1 => option1.value === value);
