...
// Toast Notification for Apex Call Error
export function serverCallError(cmp, error, errorConfig) {
    ...
    errorMsg = (errorConfig && errorConfig.message) || errorMsg || 'Unknown error';
    cmp.dispatchEvent(new ShowToastEvent( Object.assign({}, errorConfig, {message: errorMsg})));
}

export function getRecordPageRef(recordId, mode = 'view') {
    let pageRef = {
    ...
    return recordId && pageRef;
}

export function getPageRefFromInContextRef(inContextOfRef) {

    let base64Pageref = inContextOfRef.startsWith("1.") ? inContextOfRef.substring(2) : inContextOfRef;
    let decodedPageRef = base64Pageref && JSON.parse(window.atob(base64Pageref));

    return decodedPageRef;
}