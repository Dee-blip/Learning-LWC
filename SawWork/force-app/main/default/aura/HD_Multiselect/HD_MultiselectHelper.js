({
  init: function(component) {
    ...
    options.sort(function compare(a, b) {
     ...
     var selectedVals = new Set(this.getSelectedValues(component));
  
      options.forEach(function(element){
          
          if(selectedVals.has(element.value)){
              element.selected = true;
              selectedVals.delete(element.value);
          }
      });
      selectedVals.forEach(function(element){
          var temp = {};
          temp.label = element;
          temp.value = element;
          temp.selected = true;
  ...
  setInfoText: function(component, values) {
    ...
    if (values.length == 0) {
      component.set("v.infoText", "No filter applied");
    }
    if (values.length == 1) {
